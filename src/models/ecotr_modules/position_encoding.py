...
def build_position_encoding(steps, sine_type):
    N_steps = steps // 2
    if sine_type in ('lin_sine', 'exp_sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine(N_steps, normalize=True, sine_type=sine_type)
    else:
        raise ValueError(f"not supported {sine_type}")
